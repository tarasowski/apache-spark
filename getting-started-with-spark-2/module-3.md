...
# group all products based on whether they are mobiles or tables
# within each group the elements should be descending order of price
# the highest price will apply first

price_rank = (func.rank().over(windowSpec1))
# rank all producst by price within each category logical group
# the window within which the rank() function operates
...
# find the max price for products within the window specification
...
# the price difference between the highest cost product and the current product in this category
